...
def set_status(request):
    ...
    request.session.setAttribute("picture_start", picture_start);
    return HttpResponse()
...
def share_status(request):
    ...
    comment_context = str(request.GET.get('comment_context')) +"  From :" + user.user_name + " SAY: "
    ...
    status.sum = 0
    status.save()
    status1 = Status.objects.get(pk = status_id)
    picture_list = StatusPicture.objects.filter(status = status1)
    for picture in picture_list:
        picture.pk = None
